...
getContributingPointsInfoForNitrateCells <- function(nitrateCells, timeFrameOfInterest, floDataSet, allModpathStartingPoints) {
  #Troubleshooting
  dotCount <- 0
...
wisclandAnalysis <- function(nitrateCells, allModpathStartingPoints) {
  #look at wiscland data
  allWisclandData <- st_read(dsn = "//ad.wisc.edu/wgnhs/Projects/Central_Sands_Nitrate_Transport/R_Analysis/Data Sets/DS002-dataset01/CSGCC_Nitrate_Neonicotinoids.gdb", layer = "CSGCC_AG_Wiscland_Bordner_percent_per_section")
  allWisclandData <- st_transform(allWisclandData, 3070)
  dotCount <- 0
  ...
  for (nitrateCell in 1:nrow(nitrateCells)) { 
    ...
    contributingParticleList <- allModpathStartingPoints[contribPartIndexesList, ]
    
    wisclandStartingCellList <- st_within(contributingParticleList, allWisclandData)
    ...
    for (wisclandStartingCell in wisclandStartingCellList) {
      ...
      particleArea <- allWisclandData[["Area_total_km2"]][[wisclandStartingCell]]
      particlePercentAg <- allWisclandData[["Perc_AG_WISCLAND"]][[wisclandStartingCell]] #ztrey left off here. I might need to grab wisclandStargingCell[[1]]. Also, ContribPartIndex is still a vector? Not sure if that's related
  ...
  wisclandPlot <- plot(nitrateCells$Wiscland_Ag_percent, nitrateCells$MEAN_Nitrate_mg_L_0)
  wisclandPlotSummary <- lm(nitrateCells$MEAN_Nitrate_mg_L_0 ~ nitrateCells$Wiscland_Ag_percent)
...
mainNitrateCorrelator <- function(){
  ...
  nitrateCells <- getContributingPointsInfoForNitrateCells(nitrateCells,timeFrameOfInterest,floDataSet,allModpathStartingPoints)
  ...
  wisclandSummary <- wisclandAnalysis(nitrateCells, allModpathStartingPoints)
